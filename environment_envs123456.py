...
class Environment:
    ...
    def __init__(self):
        ...
        self.UPPER_STATE_BOUND        = np.array([ self.MAX_X_POSITION,  self.MAX_Y_POSITION,  4*2*np.pi, self.MAX_X_POSITION, self.MAX_Y_POSITION,  4*2*np.pi, self.MAX_X_POSITION, self.MAX_Y_POSITION]) # [m, m, rad, m, m, rad, m, m]
        ...
        self.CHECK_SOLAR_PANEL_COLLISION           = True 
        self.CHECK_END_EFFECTOR_FORBIDDEN          = True  
        self.CHECK_END_EFFECTOR_COLLISION          = True
        self.CHECK_END_EFFECTOR_SOLAR_PANEL        = True
        self.CHECK_CHASER_OBSTACLE_COLLISION       = True
        self.END_EFFECTOR_COLLISION_PENALTY        = 15.

        
        self.SUCCESSFUL_DOCKING_RADIUS    = 0.02 # [m] distance at which the magnetic docking can occur

        self.FALL_OFF_TABLE_PENALTY   =   100.
        self.END_ON_FALL              = True # end episode on a fall off the table
        self.END_ON_COLLISION         = False # end episode on collision
        self.GOAL_REWARD              =   0.
        ...
        # self.OBSTABLE_DISTANCE         = 0.45 # [m] radius of which the obstacle penalty will be applied
        self.OBSTACLE_INITIAL_POSITION = np.array([1.0, 1.0]) # [m]
        self.OBSTABLE_VELOCITY         = np.array([0. , 0.0005]) # [m/s]
        self.INITIAL_OBSTACLE_KOZ      = 1.0 # m
        self.KOZ_DECREMENT             = 0.002 # m (per TIMESTEP)
        ...
        # Solar panel shape
        self.SOLAR_PANEL_LENGTH = 0.45 # m
        self.SOLAR_PANEL_WIDTH = 0.02 # m
        self.SOLAR_PANEL_HEIGHT = 0.27 # m
        
        # Chaser shape
        self.DOCKING_PORT_MOUNT_POSITION_CHASER = np.array([0.0743, 0.20]) # [m] with respect to the centre of mass

        # Constants used for calculating physical offset of docking port in SPOT
        self.dockoffset = 0.0675; #m, offset of docking port center from COM along x-direction
        self.tarcyllength = 0.08; #m, target (BLACK) docking port cylinder length
        self.tarconelength = 0.055; #m, target (BLACK) docking port cone length

        self.cdockdeltay = self.LENGTH + self.tarcyllength + self.tarconelength; #m, cdock delta-x
        self.cdockdeltax = self.LENGTH - 2*self.dockoffset; #m, cdock delta-y
        
        self.rdock = np.sqrt(self.cdockdeltax*self.cdockdeltax + self.cdockdeltay*self.cdockdeltay); #m, cdock delta-total
        self.cdockoa = np.arctan2(self.cdockdeltay,self.cdockdeltax); #rads, cdock angle from thetat = 0

        # self.rhold = self.rdock + 0.3 # m
        # self.choldoa = np.arctan2(self.rhold,self.cdockdeltax)
    ...
    def reset(self, use_dynamics, test_time):
        ...
        if self.RANDOMIZE:
            ...
            self.state = self.NOMINAL_INITIAL_POSITION + np.random.randn(3)*[0.6, 0.6, np.pi/2]#[0.3, 0.3, np.pi/2]
            # Randomizing target state
            self.target_location = self.NOMINAL_TARGET_POSITION + np.random.randn(3)*[0.15, 0.15, np.pi/2]#[0.15, 0.15, np.pi/12]
        ...
        self.obstacle_KOZ = self.INITIAL_OBSTACLE_KOZ
        ...
        self.calculate_hold_and_docking_positions()
        ...
        if self.end_effector_collision or self.forbidden_area_collision or self.chaser_target_collision or self.chaser_obstacle_collision or self.solar_panel_collision or self.chaser_KOZ_collision or not(self.chaser_on_table):
    ...
    def step(self, action):
        ...
        # Update obstacle KOZ (only if it is larger than a platform)
        if self.obstacle_KOZ > self.LENGTH:
            self.calculate_obstacle_KOZ()
        ...
        # Grab the chaser position from the state variable
        self.chaser_position = self.state[:self.POSITION_STATE_LENGTH]
    ...
    def calculate_obstacle_KOZ(self):
        self.obstacle_KOZ -= self.KOZ_DECREMENT

    def calculate_hold_and_docking_positions(self):

        # Update the docking port target state
        self.docking_port = self.target_location + np.array([self.rdock*np.cos(self.target_location[2]+self.cdockoa), self.rdock*np.sin(self.target_location[2]+self.cdockoa), -np.pi])

        # Update the hold point location
        # self.hold_point = self.target_location + np.array([self.rhold*np.cos(self.target_location[2]+self.choldoa), self.rhold*np.sin(self.target_location[2]+self.choldoa), -np.pi])
    ...
    def reward_function(self, action):
        ...
        if np.any(np.abs(action) > self.MAX_DOCKING_SPEED):
        ...
        if self.solar_panel_collision:
            reward -= self.TARGET_COLLISION_PENALTY

        if self.chaser_obstacle_collision and self.USE_OBSTACLE:
            reward -= self.OBSTABLE_PENALTY

        if self.chaser_KOZ_collision and self.USE_OBSTACLE:
            reward -= self.OBSTABLE_PENALTY
        ...
        if (not(self.chaser_on_table) or np.abs(self.state[2]) > 6*np.pi) and self.END_ON_FALL:
    ...
    def check_collisions(self):
        """ Calculate whether the different objects are colliding with the target.
            It also checks if the chaser has fallen off the table, if the chaser has docked,
            and if it has reached the mid-way mark
        
            Returns 5 booleans: forbidden_area_collision, chaser_target_collision, chaser_on_table, mid_way, docked
        """
        ...
        C_Ib_target = self.make_C_bI(self.target_location[-1]).T        
        
        # Rotating body frame coordinates to inertial frame
        target_body_inertial = np.matmul(C_Ib_target, target_points_body) + np.array([self.target_location[0], self.target_location[1]]).reshape([2,-1])
        ...
        # Rotating body frame coordinates to inertial frame
        forbidden_area_inertial = np.matmul(C_Ib_target, forbidden_area_body) + np.array([self.target_location[0], self.target_location[1]]).reshape([2,-1])         
        forbidden_polygon = Polygon(forbidden_area_inertial.T)

        # Solar Panels on Target in body frame
        solar_panel_body = np.array([[self.LENGTH/2+self.SOLAR_PANEL_LENGTH, self.SOLAR_PANEL_WIDTH/2],   
                                        [self.LENGTH/2+self.SOLAR_PANEL_LENGTH, -self.SOLAR_PANEL_WIDTH/2],
                                        [-self.LENGTH/2-self.SOLAR_PANEL_LENGTH, -self.SOLAR_PANEL_WIDTH/2],
                                        [-self.LENGTH/2-self.SOLAR_PANEL_LENGTH, self.SOLAR_PANEL_WIDTH/2],
                                        [self.LENGTH/2+self.SOLAR_PANEL_LENGTH, self.SOLAR_PANEL_WIDTH/2]]).T        
        
        # Rotating body frame coordinates to inertial frame
        solar_panel_inertial = np.matmul(C_Ib_target, solar_panel_body) + np.array([self.target_location[0], self.target_location[1]]).reshape([2,-1])         
        solar_panel_polygon = Polygon(solar_panel_inertial.T)
        
        # Obstacle    
        obstacle_points_body = target_points_body    
        
        # Rotation matrix (body -> inertial)
        C_Ib_obstacle = self.make_C_bI(0).T        
        
        # Rotating body frame coordinates to inertial frame
        obstacle_body_inertial = np.matmul(C_Ib_obstacle, obstacle_points_body) + np.array([self.obstacle_location[0], self.obstacle_location[1]]).reshape([2,-1])
        obstacle_polygon = Polygon(obstacle_body_inertial.T)
        ...
        # Rotation matrix (body -> inertial)
        C_Ib_chaser = self.make_C_bI(self.state[2]).T        
        
        # Rotating body frame coordinates to inertial frame
        chaser_body_inertial = np.matmul(C_Ib_chaser, chaser_points_body) + np.array([self.state[0], self.state[1]]).reshape([2,-1])
        chaser_polygon = Polygon(chaser_body_inertial.T)

        # End-effector
        self.end_effector_position = np.matmul(C_Ib_chaser, np.transpose(self.DOCKING_PORT_MOUNT_POSITION_CHASER)) + self.state[:self.POSITION_STATE_LENGTH-1]
        end_effector_point = Point(self.end_effector_position)
        
        # return target_polygon, forbidden_polygon, solar_panel_polygon, obstacle_polygon, chaser_polygon, end_effector_point
        ...
        self.chaser_obstacle_collision = False
        self.solar_panel_collision = False
        self.chaser_KOZ_collision = False
        # self.mid_way = False        
        self.docked = False        
        ...
        if self.CHECK_END_EFFECTOR_FORBIDDEN and (end_effector_point.within(forbidden_polygon) or chaser_polygon.intersects(forbidden_polygon)):
            if self.test_time and self.extra_printing:
                print("Chaser/End-effector within the forbidden area!")
            self.forbidden_area_collision = True

        if self.CHECK_END_EFFECTOR_SOLAR_PANEL and end_effector_point.within(solar_panel_polygon):
            if self.test_time and self.extra_printing:
                print("End-effector collision with solar panel!")
            self.solar_panel_collision = True

        if self.CHECK_SOLAR_PANEL_COLLISION and chaser_polygon.intersects(solar_panel_polygon):
            if self.test_time and self.extra_printing:
                print("Chaser collision with solar panel")
            self.solar_panel_collision = True
        ...
        if self.CHECK_CHASER_OBSTACLE_COLLISION and chaser_polygon.intersects(obstacle_polygon):
            if self.test_time and self.extra_printing:
                print("Chaser/obstacle collision")
            self.chaser_obstacle_collision = True

        if self.CHECK_CHASER_OBSTACLE_COLLISION and end_effector_point.within(obstacle_polygon):
            if self.test_time and self.extra_printing:
                print("End-effector collision with obstacle")
            self.chaser_obstacle_collision = True

        if self.CHECK_CHASER_OBSTACLE_COLLISION and np.linalg.norm(self.state[:self.POSITION_STATE_LENGTH-1] - self.obstacle_location) <= self.obstacle_KOZ:
            if self.test_time and self.extra_printing:
                print("Chaser collision with KOZ")
            self.chaser_KOZ_collision = True

        ##########################
        ### Mid-way or docked? ###
        ##########################
        # Docking Polygon (circle)
        docking_circle = Point(self.target_location[:-1] + np.matmul(C_Ib_target, self.DOCKING_PORT_MOUNT_POSITION)).buffer(self.SUCCESSFUL_DOCKING_RADIUS)
        
        # Mid-way Polygon (circle)
        # mid_way_circle = Point(self.target_location[:-1] + np.matmul(C_Ib_target, self.DOCKING_PORT_MOUNT_POSITION)).buffer(self.MID_WAY_REWARD_RADIUS)
        ...
        ######################################
        ### Checking if chaser is on table ###
        ######################################
    ...
    def is_done(self):
        ...
        if (not(self.chaser_on_table) or np.abs(self.state[2]) > self.UPPER_STATE_BOUND[2]) and self.END_ON_FALL:
            if self.test_time and self.extra_printing:
        ...
        if self.END_ON_COLLISION and np.any([self.end_effector_collision, self.forbidden_area_collision, self.chaser_target_collision, self.solar_panel_collision, self.chaser_obstacle_collision]):
            if self.test_time and self.extra_printing:
    ...
    def make_total_state(self):
        ...
        total_state = np.concatenate([self.state[:self.POSITION_STATE_LENGTH], self.pose_error(), self.obstable_relative_location(), self.target_location, self.obstacle_location])
...
#####################################################################
##### Generating spacecraft shape for plots and animating motion ####
#####################################################################
def theSpacecraft(inputs,length,spacecraft):
    """
    ...
    r = length / 2    # half-side length of spacecraft
    wdock = 0.08                      # length of docking mechanism
    
    # Define a rotation matrix based on the chaser orientation
    rotmat = [[np.cos(theta), -np.sin(theta)],
              [np.sin(theta), np.cos(theta)]]

    #% Conversion factor
    cm2m = 1/100

    if spacecraft == "chaser":
        #% Define physical constants
        excess = 2.477*cm2m; #% m
        shrink = 1*cm2m; #% m
        bracketwidth = 6.5*cm2m; #% m
        chaselength = 5.5*cm2m; #% m
        chaseangle = 20*np.pi/180; #% rads
        ...
        sc = [[cx,   cy],
              ...
              [cx+r, cy+r],
              [cx+gamma, cy+r],
              [cx+gamma+chaselength*np.sin(chaseangle), cy+r+chaselength],
              [cx+gamma+bracketwidth-chaselength*np.sin(chaseangle), cy+r+chaselength],
              [cx+gamma+bracketwidth, cy+r],
              [cx+r, cy+r],
              [cx+r, cy]]

    elif spacecraft == "target":
        #% Define physical constants
        excess = 2.477*cm2m; #% m
        shrink = 1*cm2m; #% m
        bracketwidth = 6.5*cm2m; #% m
        ...
        #       [cx+gamma-tarconelength*np.sin(tarangle), cy+r+tarcyllength+tarconelength],
        #       [cx+gamma+bracketwidth+tarconelength*np.sin(tarangle), cy+r+tarcyllength+tarconelength],
    ...
    # rotation matrix
    sccenter = np.transpose([np.ones(len(sc))*cx, np.ones(len(sc))*cy])
    scbf = sc - sccenter
    
    # Apply the rotation matrix to each of the points
    scbf_rot = np.zeros(np.shape(scbf))
    for i in range(len(sc)):
        scbf_rot[i] = np.matmul(rotmat,scbf[i])
...
def render(states, actions, desired_pose, instantaneous_reward_log, cumulative_reward_log, critic_distributions, target_critic_distributions, projected_target_distribution, bins, loss_log, guidance_position_log, episode_number, filename, save_directory):
    ...
    chaser_state_array = states[:,0:2+1]
    # print(chaser_state_array)
    ...
    target_state_array = np.transpose(np.array([np.ones(len(theta))*desired_pose[0],np.ones(len(theta))*desired_pose[1],target_angles]))
    # print(target_state_array)
    ...
    obstac_state_array = np.transpose(np.array([obstacle_x,obstacle_y,np.zeros(len(theta))]))
    ...
    chaserline, = subfig1.plot([], [], 'r')
    # desireline, = ax.plot([], [], 'r--')
    targetline, = subfig1.plot([], [], 'k')
    obstacline, = subfig1.plot([], [], 'b')
    ...
    def initialize_axes():
        ...
        return chaserline, targetline, obstacline, time_text
    ...
    def render_one_frame(frame, *fargs):
        ...
        chaser = theSpacecraft(chaser_state_array[frame],temp_env.LENGTH,"chaser")
        target = theSpacecraft(target_state_array[frame],temp_env.LENGTH,"target")

        chaserline.set_data(np.transpose(chaser)[0],np.transpose(chaser)[1])
        targetline.set_data(np.transpose(target)[0],np.transpose(target)[1])
        ...
        if temp_env.USE_OBSTACLE:
            obstac = theSpacecraft(obstac_state_array[frame],temp_env.LENGTH,"obstacle")
            obstacline.set_data(np.transpose(obstac)[0],np.transpose(obstac)[1])
...
#            subfig1.add_patch(rotation_arrow)
    ...
    animator = animation.FuncAnimation(figure, render_one_frame, frames = np.linspace(0, len(states)-1, len(states)//5).astype(int),
